...
def load_dataset(ruta: str) -> pd.DataFrame:
    return pd.read_csv(ruta, index_col=False, compression='zip')

def clean_dataset(df: pd.DataFrame) -> pd.DataFrame:
    nuevo_df = df.copy()
    nuevo_df = nuevo_df.rename(columns={'default payment next month': 'default'})
    nuevo_df = nuevo_df.drop(columns=['ID'])
    nuevo_df = nuevo_df.loc[nuevo_df["MARRIAGE"] != 0]
    nuevo_df = nuevo_df.loc[nuevo_df["EDUCATION"] != 0]
    nuevo_df["EDUCATION"] = nuevo_df["EDUCATION"].apply(lambda x: x if x < 4 else 4)
    return nuevo_df
...
def save_model(ruta: str, estimator: GridSearchCV):
    with gzip.open(ruta, 'wb') as f:
        pickle.dump(estimator, f)

def calculate_precision_metrics(dataset_nombre: str, y, y_pred) -> dict: 
    return {
        'type': 'metrics',
...
def calculate_confusion_metrics(dataset_nombre: str, y, y_pred) -> dict:
...
def main():
    ...
    test_df = load_dataset(os.path.join(ruta1, 'test_data.csv.zip'))
    train_df = load_dataset(os.path.join(ruta1, 'train_data.csv.zip'))
    ...
    save_model(
        os.path.join(ruta2, 'model.pkl.gz'),
        estimator,
    )
